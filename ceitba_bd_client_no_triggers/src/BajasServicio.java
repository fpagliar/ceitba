...
public class BajasServicio extends JFrame implements ActionListener {
	...
	public BajasServicio(JFrame window, CeitbaConnection con) {
		this.connection = con;
		this.window = window;
	...
	public void actionPerformed(ActionEvent arg0) {
		...
		String queryString = "SELECT users.legacy, users.first_name, users.last_name, services.name, services.price, subscriptions_history.start, subscriptions_history.end "
				+ "FROM subscriptions_history JOIN services ON subscriptions_history.service_id = services.id JOIN users ON subscriptions_history.user_id = users.id "
				+ "WHERE services.name='" + name + "' AND subscriptions_history.start >= '"
				+ startY
				+ "-"
				+ startM
				+ "-"
				+ startD
				+ "' AND subscriptions_history.end <= '"
				+ endY
				+ "-"
				+ endM
				+ "-" + endD + "' " + "ORDER BY users.legacy ";
		...
		try{
			Date date = new Date();
			ExcelExport export = new ExcelExport(date.getDay() + "-" + date.getMonth() + "-" + date.getYear() + "_Bajas_entre_" + startD + "-" + startM + "-" + startY + "_y_" + endD + "-" + endM + "-" + endY + "_de_" + name);
